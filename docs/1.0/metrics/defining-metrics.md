...
ممکن است یک metric را برای نمایش تعداد کل کاربران اضافه شده به درخواست شما در هر روز یا میزان فروش هفتگی، تعریف کنید

 سه نوع معیار ساخته شده: Value، Trend و Partition. ما هر نوع metric را بررسی خواهیم کرد و استفاده خود را در زیر نشان خواهیم داد.

## Value Metrics

مقدار کلی رو نشون می ده و مقدار رو نسبت به چند روز گذشته رو نشون می ده که کاهش داشته یا افزایش.

![Value Metric](./img/value.png)

Value metrics may be generated using the `Zoroaster:value` Artisan command. By default, all new metrics will be placed in the `app/Zoroaster/Metrics` directory:

```bash
php artisan Zoroaster:value NewUsers
```
...
```bash
php artisan Zoroaster:trend UsersPerDay
```
...
```bash
php artisan Zoroaster:partition UsersPerPlan
```
